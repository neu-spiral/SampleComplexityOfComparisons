...
def generate_comparison_fixed_degree(m, degree, seed):
    ...
    print("degree:"+str(degree)+" n:"+str(m))
    ...
    def estimate(self, error_method, algorithm, estimate_method, noise_level, correct_method, cv_fold_index = None):
        ...
        if 'Oracle' in algorithm:
            ...
            if len(oracle_name_list)==1:
                ...
                error_oracle = np.linalg.norm(beta_oracle - self.cbeta)
                error_normalized_oracle = self.beta_estimate_error(beta_oracle, self.beta)
            ...
            error_list.append(((error_no_correction*1,error_normalized_no_correction*1),test_auc*1, test_noiseless_auc*1))
